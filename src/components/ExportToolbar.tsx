...
export default function ExportToolbar({ locale }: ExportToolbarProps) {
  const { filename, sourceMarkdown, generatedHtml, quiz } = useQuizStore();
  const disabled = !quiz;
  const copy = siteCopy[locale].exportBar;
    ...
    <section className="rounded-[28px] border border-white/10 bg-slate-950/35 p-6 shadow-[0_20px_48px_rgba(15,23,42,0.18)]">
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <h3 className="mt-2 text-xl font-bold text-white">{copy.title}</h3>
        </div>
